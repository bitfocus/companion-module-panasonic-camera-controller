// Ported from companion-module-panasonic-cameras, where src/auth.js is maintained alongside this
// module's identical copy. Kept diffable against it — including the wording, which speaks of the
// cameras those measurements were taken on — so a case added on either side can be copied across.
// What is adapted is the request target: this module's commands go to aw_cam, not aw_ptz.
...
describe('buildDigestAuthorization', () => {
	...
	// The command lives in the query, so hashing the path alone — or a re-encoded one — yields a
	// response the device refuses for ever. This module's commands carry a colon (XQC:01), which is
	// legal in a query and which the URL parser therefore leaves alone: what is hashed here has to
	// be what went on the wire.
	it('hashes the request target including its query, exactly as sent', () => {
		const url = new URL('http://10.0.0.1/cgi-bin/aw_cam?cmd=XQC:01&res=1')
		const uri = url.pathname + url.search

		expect(uri).toBe('/cgi-bin/aw_cam?cmd=XQC:01&res=1')
		...
		expect(header).toContain('uri="/cgi-bin/aw_cam?cmd=XQC:01&res=1"')
	...
	// A percent-encoded target is hashed as it stands, decoding nothing. No command this module sends
	// carries one, but the sibling module's do (aw_ptz?cmd=%23O), and this file is shared with it.
	it('leaves a percent-encoded query encoded', () => {
		const header = buildDigestAuthorization(digest(PANASONIC), {
			username: 'admin',
			password: '12345',
			method: 'GET',
			uri: '/cgi-bin/aw_ptz?cmd=%23O&res=1',
			nc: 1,
			cnonce: '0a4f113b',
		})
